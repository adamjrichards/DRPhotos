<?php
	function ptf_parser( $photo_folder ) {
		...
		$ptf_text_array = preg_split( "#[/\^(\{\#\})()]#", substr( $ptf_text_file, strstr( $ptf_text_file, "BEGIN CONTENT", FALSE ) ) );
		$ptf_data_array = array();
							    
		function clean_up_ptf( $ptf_text_file ) {
			$ptf_text_string = substr( $ptf_text_file, 0, strstr( $ptf_text_file, ".htm\n", TRUE ) );
			$ptf_text_string = preg_rep( $ptf_text_string, "/[\/\*\^\{\}\#]/", "" );
